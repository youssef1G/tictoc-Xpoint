...
export default function ImageUploader({ images = [], onChange }) {
  ...
  async function uploadOne(file) {
    if (file.size > 10 * 1024 * 1024) throw new Error(`${file.name} ${t('uploader.sizeError')}`)
    ...
    if (!res.ok) throw new Error(t('uploader.uploadFail'))
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="grid grid-cols-4 gap-3">
          {images.map((url, idx) => (
            <div key={url + idx} className="relative group rounded-xl overflow-hidden border border-[var(--border)] aspect-square">
              ...
              {idx === 0 && (
                <span className="absolute top-1 left-1 bg-[var(--brand)] text-white text-[9px] font-semibold px-1.5 py-0.5 rounded-[4px]">{t('uploader.main')}</span>
              )}
      ...
      <div onClick={() => inputRef.current?.click()}
        ...
        {uploading ? (
          <p className="text-xs text-[var(--muted)]">{t('uploader.uploading')}</p>
        ) : (
          <>
            <p className="text-xs font-semibold text-[var(--text)]">{t('uploader.click')}</p>
            <p className="text-[11px] text-[var(--muted)] mt-1">{t('uploader.multipleText')}</p>
          </>
        )}
      </div>
      ...
      {images.length > 0 && (
        <p className="text-[11px] text-[var(--muted)]">{t('uploader.mainPhoto')}</p>
      )}
    </div>
  )
}
